...
- **Catppuccin Mocha for yazi**, the last tool in the set still on its own
  ...
  deployed whole to `%APPDATA%\yazi\config\flavors\` - yazi's own location on Windows, not the XDG path the zsh platforms use. What catppuccin publishes for yazi instead is a
...
- **`--doctor` checks the flavor**, the two files and the key that names them
  separately, alongside the k9s and lazygit checks.
...
- **`gb`, `gs` and `cheat` in the PowerShell profile.** `gb` switches branch -
  local and remote, newest first, the log as the preview, `--track` for a
  remote one. `gs` browses stashes with the diff (through delta) as the
  preview: Enter applies, ctrl-p pops, ctrl-x drops. `cheat` searches the
  `tools` list and shows the pick's tldr page; unlike zsh it cannot put the
  command on the prompt, since a running function has no way to type into it.
  `fkill` is not redefined - PSFzf already provides it (`-EnableAliasFuzzyKillProcess`).
  `gs` is not defined where Ghostscript already owns the name.
- **`-Doctor` checks that `tools` and the pickers loaded**, in a shell that
  loads the profile.
- **`-Doctor` reports the tldr page cache.** Absent is a note, since the
  first `tldr` fills it; nothing under 30 days old is a note too. It reads
  `%LOCALAPPDATA%\tealdeer\cache` rather than running `tldr`, whose
  `--show-paths` would download the pages.

### Changed

- **`tools-list.ps1` is generated by `New-ToolsListContent`**, apart from the
  file and result handling, and keeps its rows in `$global:ToolsRows` for
  `cheat`. It is tested with a description made of an apostrophe, `&`, a
  backslash and `$(...)`.
