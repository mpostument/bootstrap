...
- **`--doctor` checks the flavor**, the two files and the key that names them
  separately, the same three lines as Linux.
...
- **Four fzf pickers in zsh**, the same as on Linux: `gb` (branch switcher),
  `gs` (stash browser - Enter applies, ctrl-p pops, ctrl-x drops), `fkill`
  (pick your own processes, Tab for several) and `cheat` (search the `tools`
  list, preview the tldr page, command onto the prompt). `fkill` lists with
  `ps -U`, not `-u`: on macOS `-u` is an output format, not a user filter.
  `gs` is not defined where Ghostscript already owns the name.
...
- **`--doctor` reports the tldr page cache.** Absent is a note, since the
  first `tldr` fills it; nothing under 30 days old is a note too. It looks in
  both `~/.cache` and `~/Library/Caches` - the config moved to XDG in tealdeer
  1.9, the cache did not - and reads the directory rather than running `tldr`,
  whose `--show-paths` would download the pages.

### Changed

- **`tools` is generated by one function, `emit_tools_function`**, and leaves
  its rows in `_TOOLS_ROWS` for `cheat`.
