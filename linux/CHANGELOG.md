...
- **Four fzf pickers in zsh.** `gb` switches branch - local and remote, newest
  commit first, the log as the preview; picking a remote branch checks it out
  with `--track`. `gs` browses stashes with the diff (through delta) as the
  preview: Enter applies, ctrl-p pops, ctrl-x drops - apply is the default
  because it leaves the stash behind. `fkill [signal]` lists your own
  processes, Tab marks several, and prints the `kill` line it runs. `cheat`
  searches the `tools` list, previews the command's tldr page, and puts the
  chosen command on the prompt with `print -z`. Each only picks; what runs
  afterwards is an ordinary git or kill command. `gs` is not defined where
  Ghostscript already owns the name.
- **`--doctor` checks that `tools` and the pickers loaded**, in a login shell:
  a fragment that is deployed but never sourced, or a list that generated
  empty, now says so instead of surfacing as a missing command.
- **`--doctor` checks that every release binary launches.** On PATH at the
  right path is not the same as runnable: a wrong-architecture binary or a
  missing library passed every earlier check. It asks each for `--version` and
  treats any answer, even "unknown option", as launched; only 126, 127 and a
  crash (132, 134, 139) are broken.
- **`--doctor` reports the tldr page cache.** Absent is a note, since the
  first `tldr` fills it (`auto_update`); nothing under 30 days old is a note
  too. It reads the directory rather than running `tldr`, whose
  `--show-paths` would download the pages.

### Changed

- **`tools` is generated by one function, `emit_tools_function`**, and leaves
  its rows in `_TOOLS_ROWS` for `cheat`. It is tested with a description made
  of an apostrophe, `&`, a backslash and `$(...)`.
- **`tools/test.sh` covers the `tools` function, the pickers, and the new
  doctor checks** - including a scratch git repo driving `gb` and `gs`.
